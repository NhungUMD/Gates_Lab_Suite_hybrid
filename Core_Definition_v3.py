...
global s
s=[[[1,0],[0,1]],[[0,1],[1,0]],[[0,-1J],[1J,0]],[[1,0],[0,-1]]]
...
class Quantum_State:
    ...
    def Import(self,filename,**kwarg):
        ...
        for i in range(len(self.population)):
            self.error_lower[i],self.error_upper[i]=error(self.population[i],temp_sum)
        self.population=np.divide(self.population,temp_sum)
        read_file.close()
    ...
    def Mean_Value(self,function):
        mean_value=0
        for i in range(len(self.population)):
            mean_value+=self.population[i]*function(Binary_Decoding(i,self.size))
        return mean_value
    
    def Std(self,function):
        std=0
        for i in range(len(self.population)):
            std+=np.square((self.error_upper[i]-self.error_lower[i])/2)*np.square(function(Binary_Decoding(i,self.size)))
        return np.sqrt(std)
...
class Quantum_Gate:
    ...
    def Matrix_Representation(self,size):
        ...
        if self.name=="XA":
            ...
            temp=np.matmul(XX_Rotation(size,self.qubit1,self.qubit2,param*pi),temp)
            ...
            temp=np.matmul(XX_Rotation(size,self.qubit1,self.qubit2,pi/4),temp)
            temp=np.matmul(Y_Rotation(size,self.qubit1,-pi/2),temp)
    ...
    def GatesLab_Command(self):
        ...
        if self.name.find("SKA")>-1 and self.name.find("SKAAZY")==-1 :
            ...
            section=self.name+str(mapping[self.qubit1])+str(mapping[self.qubit2])
            ...
            if mapping[self.qubit1]<mapping[self.qubit2]:
                ...
                section=self.name+str(mapping[self.qubit2])+str(mapping[self.qubit1])
            if param>0:
                section+="+"
            else:
                section+="-"
...
#                 param2=float(command[13:19])
                ...
                self.Add_Gate(Quantum_Gate(name,qubit1,angle=param,axis=axis))
            ...
            self.gates[self.depth-1].qubit1=mapping_back[self.gates[self.depth-1].qubit1]
            if self.gates[self.depth-1].qubit2!=None:
                self.gates[self.depth-1].qubit2=mapping_back[self.gates[self.depth-1].qubit2]
        ...
        if (self.gates[self.depth-1].qubit1>=self.size)or(self.gates[self.depth-1].qubit2!=None and self.gates[self.depth-1].qubit2>=self.size):
            raise ValueError("number of qubit involved in the circuit is more than that defined for the circuit.")
        return
